...
import net.trilogy.arch.adapter.DatabaseAdapter;
...
public class ImportCommand implements Callable<Integer> {
    ...
    public Integer call() throws Exception {
        ...
        File yamlOutputFile = this.productDocumentationRoot.toPath().resolve("data-structure.yml").toFile();
        File exportedFile = new ArchitectureDataStructureWriter().export(dataStructure, yamlOutputFile);
        logger.info(String.format("Architecture data structure written to - %s", exportedFile.getAbsolutePath()));

        DatabaseAdapter db = DatabaseAdapter.load();
        db.write(dataStructure);
        File csvOutputFile = this.productDocumentationRoot.toPath().resolve("architecture.csv").toFile();
        File exportedCsv = db.export(csvOutputFile);
        db.close();
